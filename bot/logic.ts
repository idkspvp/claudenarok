// Pure (IO-free) logic for the Claudenarok Online Discord bot: Gateway intents,
...
export const SLASH_COMMANDS = [
  { name: 'whoami', description: 'Show your Claudenarok Online link status and reward points' },
  { name: 'link', description: 'Get the link to connect your Discord to Claudenarok Online' },
] as const;
...
export function buildWhoamiContent(roles: {
  ...
  if (!roles.linked) {
    return 'Your Discord is not linked to a Claudenarok Online account yet. Use /link to connect it and start earning rewards.';
  }
...
export function buildLinkContent(gameUrl: string): string {
  return `Connect your Discord to Claudenarok Online to earn rewards and flex your characters: open ${gameUrl}, log in, and press the Discord button in the game HUD (or "Continue with Discord" on the login screen).`;
}

/** Welcome message for a new guild member. */
export function buildWelcomeMessage(opts: { userMention: string; gameUrl: string }): string {
  return `Welcome to Claudenarok Online, ${opts.userMention}! Play at ${opts.gameUrl} and link your Discord in the game HUD to earn rewards, claim swag, and rank up here in the server.`;
}
...
export function buildRelayMessage(item: RelayItem, gameUrl: string): Record<string, unknown> {
  ...
  const embed: Record<string, unknown> = {
    ...
    footer: { text: 'Claudenarok Online' },
  };
