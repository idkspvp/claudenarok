...
const baseEnTable = {
  ...
  'log.socketed': 'Socketed {card} into {item}.',
  'error.socketEmptySlot': 'Nothing is equipped in that slot.',
  'error.socketNotACard': 'That is not a card.',
  'error.socketNotHeld': 'You do not have that card.',
  'error.socketNoSockets': '{item} has no card sockets.',
  'error.socketFull': '{item} has no free card socket.',
  'error.socketWrongSlot': '{card} does not fit {item}.',
...
const RULES: Rule[] = [
  ...
  {
    re: /^Socketed (.+) into (.+)\.$/,
    build: (m) => tSim('log.socketed', { card: locItem(m[1]), item: locItem(m[2]) }),
  },
  { re: /^Nothing is equipped in that slot\.$/, build: () => tSim('error.socketEmptySlot', {}) },
  { re: /^That is not a card\.$/, build: () => tSim('error.socketNotACard', {}) },
  { re: /^You do not have that card\.$/, build: () => tSim('error.socketNotHeld', {}) },
  {
    re: /^(.+) has no card sockets\.$/,
    build: (m) => tSim('error.socketNoSockets', { item: locItem(m[1]) }),
  },
  {
    re: /^(.+) has no free card socket\.$/,
    build: (m) => tSim('error.socketFull', { item: locItem(m[1]) }),
  },
  {
    re: /^(.+) does not fit (.+)\.$/,
    build: (m) => tSim('error.socketWrongSlot', { card: locItem(m[1]), item: locItem(m[2]) }),
  },
