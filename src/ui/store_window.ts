...
export interface StoreWindowDeps {
  root(): HTMLElement;
  world(): IWorld;
  closeOthers(): void;
...
export class StoreWindow {
  ...
  constructor(private readonly deps: StoreWindowDeps) {}
  ...
  toggle(): void {
    ...
    this.poll = window.setInterval(() => {
      if (this.isOpen) void this.renderStore(null);
    }, 15_000);
  }
  ...
  private ensureShell(): void {
    const root = this.deps.root();
    markDialogRoot(root, { labelledBy: 'store-window-title' });
    if (root.querySelector('.woc-store-body')) return;
    root.innerHTML = this.titleHtml() + this.loadingStripHtml() + this.loadingHtml();
    root.querySelector('[data-close]')?.addEventListener('click', () => this.close());
    root.classList.add('store-active');
  }
  ...
  private async renderStore(focus: 'open' | null): Promise<void> {
    ...
    if (!body) return;
    if (focus === 'open') root.querySelector<HTMLElement>('[data-close]')?.focus();
    ...
    if (this.isOpen) this.paintStore(body);
  }
  ...
  onCosmeticsChanged(): void {
    if (!this.isOpen || !this.storeReady) return;
    this.rebuildArmorySections();
  ...
  private afterArmoryChange(skinId: string): void {
    this.rebuildArmorySections();
    const body = this.deps.root().querySelector<HTMLElement>('.dr-body');
    if (body && this.isOpen) this.paintStore(body);
  ...
  private titleHtml(): string {
    return (
      `<div class="panel-title"><span id="store-window-title">${esc(t('hudChrome.wocStore.title'))}</span>` +
      `<button type="button" class="x-btn" data-close aria-label="${esc(t('hudChrome.wocStore.close'))}">${svgIcon('close')}</button></div>`
    );
  }

  /** The former tab strip is now just the busy indicator's home: the window has
   *  a single view since the rewards tab left with the wallet surface. */
  private loadingStripHtml(): string {
    return (
  ...
  private loadingHtml(): string {
    ...
    // indicator already announces busy.
    const spinner = (label: string, live: boolean): string =>
      ...
      `<span>${esc(label)}</span>` +
      `</div>`;
    return `<div id="woc-store-panel" class="dr-body woc-store-body">${spinner(t('hudChrome.wocStore.loading'), true)}</div>`;
  }
