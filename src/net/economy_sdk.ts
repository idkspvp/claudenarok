...
// Card is the only purchase rail; the crypto rails left with the wallet surface.
...
/**
 * Optional client-side signer for the card rail. main.ts passes this once the live
 * integration exists; until then it is absent and the flow stops cleanly after the
 * server intent (no crash, nothing charged): hand the returned clientSecret +
 * publishableKey to Stripe.js and confirm the PaymentIntent client-side.
 */
