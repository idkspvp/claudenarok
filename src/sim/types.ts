...
//
// Ragnarok has NO divisor here: a swing is the weapon roll plus status ATK, and
// attack speed is a separate stat that does not scale either of them. The player
// side works that way now (combat/weapon_damage.ts), so STATUS_AP_PER_DPS is
// gone rather than kept at 1.
//
// MOB_AP_PER_DPS survives for exactly one reason. Monster templates still carry
// attack power on the pre-conversion scale, where a level-20 attacker had a few
// hundred, so adding it raw would multiply monster damage by more than ten.
// Rescaling every monster record is content work, and the Ragnarok conversion
// replaces those records outright with an authored ATK pair, at which point the
// pair IS the damage and this constant goes with it. Until then monster damage
