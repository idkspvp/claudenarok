...
// ZONES + DUNGEONS + the overworld bestiary), so the Guide's data never
...
// numbers, mechanic names, loot, the raid boss name, or per-encounter scripts.
...
const classes = ALL_CLASSES.map((id) => {
  ...
  const roles = ROLE_ORDER.filter((r) => FINDER_CLASS_ROLES[r].includes(id));
...
// Ability names are the English sim source (proper nouns); all other Guide copy is
// localized via guide.* t() keys. No balance numbers or instanced spoilers here.
