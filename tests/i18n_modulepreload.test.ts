...
describe('i18n modulepreload build hook', () => {
  describe('parseSupportedLocales', () => {
    ...
    it('throws on a genuinely empty array, but NOT on an English-only one', () => {
      // The distinction the build turns on. An empty array means the generator
      // produced nothing and the build must stop. An array holding only 'en' is
      // the game as it currently ships: one locale, no lazy chunks, nothing to
      // preload. Reading the second as the first failed the whole build.
      expect(() =>
        parseSupportedLocales('export const SUPPORTED_LANGUAGES = [] as const;'),
      ).toThrow(/empty locale set/);
      expect(parseSupportedLocales("export const SUPPORTED_LANGUAGES = ['en'] as const;")).toEqual(
        [],
      );
      expect(
        parseSupportedLocales("export const SUPPORTED_LANGUAGES = ['en', 'es'] as const;"),
      ).toEqual(['es']);
    });
  });
