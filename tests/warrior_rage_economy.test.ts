...
// rage-positive, Anger Management added +25%/+15%, and swordman auto-attacks ran
...
function furyWarrior(): AnySim {
  const sim = new Sim({ seed: 2712, playerClass: 'swordman' }) as AnySim;
...
describe('v0.27.1 swordman rage economy', () => {
  it('swordman auto-attack rage is back on the classic 7.5x outgoing scale', () => {
    ...
    // The literal 7.5 is the pin: the talents-v2 era ran a swordman-only 9x here.
  ...
  // The Anger Management row test that stood here read a swordman choice row,
