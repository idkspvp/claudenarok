...
describe('Aspect of the Monkey', () => {
  it('is a archer nature self-buff learned at level 5', () => {
    const def = ABILITIES['aspect_of_the_monkey'];
    expect(def).toBeTruthy();
    expect(def.class).toBe('archer');
  ...
  it('is unknown at level 4 and known from level 5 in the archer kit', () => {
    const at4 = abilitiesKnownAt('archer', 4).map((k) => k.def.id);
    const at5 = abilitiesKnownAt('archer', 5).map((k) => k.def.id);
    expect(at4).not.toContain('aspect_of_the_monkey');
    expect(at5).toContain('aspect_of_the_monkey');
  });

  it('raises the archer dodge chance by 8% when cast', () => {
    const sim = new Sim({ seed: 42, playerClass: 'archer', autoEquip: true });
