...
function stubEntity(partial: Partial<Entity> & Pick<Entity, 'id' | 'kind'>): Entity {
  return {
    ...
    maxResource: 0,
    resourceType: null,
    stats: { str: 0, agi: 0, vit: 0, int: 0, luk: 0 },
...
describe('activePvpOpponentIds', () => {
    ...
    const ids = activePvpOpponentIds({
      ...
      arenaInfo: {
        ...
        match: {
          ...
          allies: [],
          enemies: [
            { pid: 3, name: 'Arena Rival', cls: 'swordman', level: 1 },
  ...
  it('includes the ENEMY Yumi cat entity id, never the own cat', () => {
    ...
    const matchBase = {
      ...
      oppPid: 3,
      oppName: 'Rivals',
      oppClass: 'swordman' as const,
      oppLevel: 1,
      state: 'active' as const,
      format: 'yumi3' as const,
      allies: [],
      enemies: [{ pid: 3, name: 'Rival', cls: 'swordman' as const, level: 1 }],
    };
    const yumi = (team: 'A' | 'B') => ({
      team,
      size: 3 as const,
