...
describe('provider usage metrics', () => {
  ...
  it('reports rolling counts across the admin dashboard windows', () => {
    ...
    recordUsageMetric('github.contributors.fetch', now - 25 * 60 * 60_000);
    recordUsageMetric('github.contributors.fetch', now - 2 * 60 * 60_000);
    recordUsageMetric('github.contributors.fetch', now - 2 * 60_000);
    recordUsageMetric('github.contributors.fetch', now - 30_000);

    expect(metricCounts('github.contributors.fetch', now)).toEqual({
      m1: 1,
      m5: 2,
      h1: 2,
      h24: 3,
  ...
  it('reports cache counters and current cache size', () => {
    ...
    setUsageCacheSize('github.contributors', 3, 1024, now);
    recordUsageCacheEvent('github.contributors', 'hit', now);
    recordUsageCacheEvent('github.contributors', 'miss', now);
    recordUsageCacheEvent('github.contributors', 'stale', now);
    recordUsageCacheEvent('github.contributors', 'store', now);
    recordUsageCacheEvent('github.contributors', 'failure', now);
    recordUsageCacheEvent('github.contributors', 'eviction', now);

    const cache = providerUsageSnapshot(now).caches.find(
      (row) => row.key === 'github.contributors',
    );
