...
## Monsters run on a different rule, and it is knowable

The Max HP formula's own note says a monster that is not a summon is handed
straight to a monster rule instead. That rule is not in `mechanics.json`, but it
is in the wiki's page source: the site computes and displays every monster's
health, defence, accuracy and evasion from `monStats()`, and renders a prose
tooltip beside the numbers that states the same thing independently.

```
slv       = boss ? level*2 : level          bosses compute attributes at DOUBLE level
attr      = max(0, round(combat.<attr> * slv))          the six attributes
dmf       = boss ? 3 : 1                     the boss Def/MdefMult block, +200%

def       = round( round(combat.def  / 10 * level) * dmf )
mdef      = round( round(combat.mdef / 10 * level) * dmf )
defFlat   = round( (floor(level/2) + floor(vit/5)) * dmf )
mdefFlat  = round( (floor(level/4) + floor(vit/10) + floor(int/5)) * dmf )

hp        = roundSig2( (100 + 10*level + 10*Tri(level)) * (1 + vit/100)
                       * (boss ? 101 : 1) * (1 + 0.25*clamp01((level-1)/149)) )

hit       = floor(level + dex/2)
flee      = floor(level + agi/2)

exp, coin = a boss kill pays 50x; coin = level * 3
```

Four things follow, and the first corrects an earlier reading in this
directory:

- **The archetype name is a label, nothing more.** The site prints
  `combat.arch` as a tag and never reads it numerically. The multipliers apply
  straight to level, so the twelve monster archetypes needing definitions was a
  false alarm: there is nothing to define.
- **Monster health is the same quadratic as a player's** (`10*Tri(level)`) but
  with a flat 100 base instead of a class multiplier, plus a late-game ramp
  worth up to +25% by level 150. `roundSig2` then rounds to two significant
  figures, which is why every published monster health is a round number.
- **A boss is roughly 120x a same-level mob**, from the 101x health multiplier
  compounded with attributes computed at double level. Not a separately authored
  stat block: the same row with two switches flipped.
- **Monster defence has no weapon or gear term**, just `combat.def / 10 * level`,
  so a monster's soft defence is a straight tenth of its growth number per level.

The prose tooltip omits the late-game ramp that the code applies. Where they
disagree the code is recorded, since it is what produces the displayed numbers.

The computed table for all 319 monsters is
`data/spiritvale-monster-stats.tsv`, generated by running this rule over
`monsters.json` rather than transcribed.

**Still not derivable:** monster ATK (the tooltip says it scales with weapon and
rank, and the site displays no ATK), and the `AttackSpeedRanks` values behind the
`ms` and `as` fields, which are ranks out of 5.
