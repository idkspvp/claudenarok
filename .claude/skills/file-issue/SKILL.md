---
name: file-issue
description: Create a GitHub issue for Claudenarok Online in the maintainer's house format. Use when asked to file, open, or create a GitHub issue or bug report from a description, a screenshot, or a rough placeholder. Rewrites the input into a clean, professional issue (Problem, Steps to reproduce, Expected, Actual, Scope, Acceptance criteria) and includes a Screenshot section only for UI/UX issues. Posts to idkspvp/claudenarok via gh.
user-invocable: true
---

# File an issue (Claudenarok Online house style)
...
`idkspvp/claudenarok`, never a fork.

```
gh issue create \
  --repo idkspvp/claudenarok \
  --title "<title>" \
