...
#
# Claudenarok Online ships a pre-push QA floor in .githooks/. Git does not use a checked-in
...
if [ -z "$current" ]; then
  if git config --local core.hooksPath .githooks 2>/dev/null; then
    echo "Claudenarok Online: enabled .githooks (pre-push QA floor: tsc, guard tests, biome, copy rules). Undo with: git config --unset core.hooksPath" >&2
  fi
fi
exit 0
